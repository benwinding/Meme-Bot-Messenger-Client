...
app.post("/webhook", function (req, res) {
  ...
  data.entry.forEach(function (entry) {
    ...
    entry.messaging.forEach(function (event) {
      ...
      if (message) Bot.HandleMessageRecieved(senderId, message);
      else if (postback) Bot.HandlePostBackRecieved(senderId, postback);
      else hlpr.err("Webhook unknown event not a message or postback: ", event);
    });
  });
  res.sendStatus(200);
});
